...
public class Flocker extends Follower {
  ...
  /*  Start with a new force with weight 1 and heading 0 (straight ahead)
    ...
    protected WeightedForce maintainClearance(List<Percept> ps) {
        ...
        for(Percept p : ps)
        {
        	if (p.getDistance() <= flocking.clearance && flocking.cone > p.getAngle() && flocking.cone*-1 < p.getAngle() && (p.getObjectCategory() == Percept.ObjectCategory.OBSTACLE || p.getObjectCategory() == Percept.ObjectCategory.PREDATOR))
        	{
        			//if clearance and Distance are the same then will be obstacle weight
        			if(p.getAngle() > 0)
        			{
        				mf.addIn(new WeightedForce(flocking.obstacleWeight*(flocking.clearance/p.getDistance()),flocking.cone*-1));
        			}
        			else
        			{
        				mf.addIn(new WeightedForce(flocking.obstacleWeight*(flocking.clearance/p.getDistance()),flocking.cone) );
        			}
        	}
        }
        return mf;
    ...
    protected WeightedForce separateFromNeighbors(List<Percept> ps) {
        ...
        for(Percept p : ps)
        {
        	if (p.getDistance() <= flocking.separationDistance && p.getObjectCategory() == Percept.ObjectCategory.BOID)
        	{
        		sf.addIn(new WeightedForce(flocking.separationWeight*(flocking.separationDistance/p.getDistance()),p.getAngle()*-1) );
        	}
        }
        return sf;
    }
    ...
    protected WeightedForce alignWithNeighbors(List<Percept> ps) {
       ...
       // double orientation = 0 ;
        for(Percept p : ps)
        {
        	System.out.println("boid angle is :"+p.getAngle());
        	System.out.println("your angle is :"+flocking.cone);
        	if (p.getDistance() < flocking.detectionDistance && p.getDistance() > flocking.separationDistance && p.getObjectCategory() == Percept.ObjectCategory.BOID)
        	{
        		boidCount++;
        		af.addIn(new WeightedForce(flocking.alignmentWeight,p.getOrientation()));
        	}
        }
        if(boidCount != 0)
        {
        	af.reweight(1.0/boidCount);
        }
        return af;
    }
    ...
    protected WeightedForce centerOnNeighbors(List<Percept> ps) {
        ...
        System.out.println("count "+ boidCount);
        if(boidCount != 0)
        {
        	cf.reweight(1.0/boidCount);
        }
        return cf;
    }
...
//Inversely prop to the distance to the light
    	 for(Percept p : ps)
         {
    		 	if (p.getDistance() <= flocking.detectionDistance && flocking.cone > p.getAngle() && flocking.cone*-1 < p.getAngle() && p.getObjectCategory() == Percept.ObjectCategory.LIGHT)
            	{
    		 		ff.addIn(new WeightedForce( flocking.followWeight*(flocking.detectionDistance/p.getDistance()),p.getAngle()));
            	}
         }
    	return ff;
    }
    
    protected WeightedForce avoidZombies(List<Percept> ps) {
    	WeightedForce az = new WeightedForce();
//Inversely prop to the distance to the light
    	 for(Percept p : ps)
         {
    		 	if (p.getDistance() <= flocking.detectionDistance && flocking.cone > p.getAngle() && flocking.cone*-1 < p.getAngle() && p.getObjectCategory() == Percept.ObjectCategory.CORPSE)
            	{
    		 		az.addIn(new WeightedForce( flocking.obstacleWeight*4*(flocking.detectionDistance/p.getDistance()),p.getAngle()*-1));
            	}
         }
    	return az;
    }
	...
	// Calculate all the forces required for this boid
        ...
        WeightedForce zombieForce;
        if (flocking.avoidsZombies) {
        	zombieForce = avoidZombies(ps);
        }
        else {
        	zombieForce = new WeightedForce();
        }
